...
!function($) {
    ...
    var createFormAvd = function(pColumns, searchText, that) {
        ...
        for (var i in pColumns) {
            ...
            if (!vObjCol.checkbox && vObjCol.visible && vObjCol.searchable) {
				var str=JSON.stringify(vObjCol);
				//modified by Hebin,适应选项搜索
				if(str.indexOf("source")>0){
					htmlForm.push('<div class="form-group">');
					htmlForm.push(sprintf('<label class="col-sm-4 control-label">%s</label>', vObjCol.title));
					htmlForm.push('<div class="col-sm-6">');
					htmlForm.push(sprintf('<select class="form-control select-md" name="%s" placeholder="%s" id="%s">', vObjCol.field, vObjCol.title, vObjCol.field));
					var optionsstr=vObjCol.editable.source.replace(/'/g,'"');
					var options=JSON.parse(optionsstr);
					htmlForm.push('<option value=""></option>');
					var i=0;
					for(var one in options){
						if(i>0){
							htmlForm.push(sprintf('<option value="%s">%s', one, options[one]));
							htmlForm.push('</option>');
						}
						i++;
					}
				...
				//console.log(JSON.stringify(vObjCol));
            }
        }
    ...
    BootstrapTable.prototype.onColumnAdvancedSearch = function (event) {
		...
		//$(event.currentTarget).is('input') 查询选择器
		if (text) {
